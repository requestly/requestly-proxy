...
const process_modify_response_action = (action, ctx) => {
    ...
    if (action.responseType &&
        action.responseType === requestly_master_1.CONSTANTS.RESPONSE_BODY_TYPES.CODE) {
        modify_response_using_code(action, ctx);
        return (0, utils_1.build_action_processor_response)(action, true);
    }
    else if (action.responseType === requestly_master_1.CONSTANTS.RESPONSE_BODY_TYPES.LOCAL_FILE) {
...
const modify_response_using_code = (action, ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        userFunction = requestly_master_1.UTILS.GET_FUNCTION_FROM_STRING(action.response);
    }
    catch (error) {
